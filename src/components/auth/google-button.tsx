...
/** Managed Google sign-in. Always returns to a public same-origin URL. */
export function GoogleButton({ label = "Continue with Google" }: { label?: string }) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (result.error) {
        toast.error("Google sign-in failed", String(result.error));
        setLoading(false);
        return;
      }
      ...
      toast.error("Google sign-in failed", error instanceof Error ? error.message : undefined);
      setLoading(false);
    }
  };
